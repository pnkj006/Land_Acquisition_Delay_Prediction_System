...
export default function Messages() {
  ...
  const { notifications, readIds, markAllRead, markAsRead, markAsUnread, timeAgo } = useNotifications()
  ...
  const messages = useMemo(() => { const projectById = new Map(projects.map((project) => [project.id, project])); return notifications.map((notification) => ({ ...notification, project: projectById.get(notification.projectId) || null, sender: SENDERS[notification.type] || 'System notification', read: readIds.has(notification.id) })) }, [notifications, projects, readIds])
  ...
  const selectMessage = (message) => { setSelectedId(message.id); markAsRead(message.id) }
  ...
  return (
    <DashboardLayout activeKey="messages">
      ...
      <div className="mb-4 flex flex-wrap gap-3"><FilterDropdown options={CATEGORIES} value={filters.category} onChange={(value) => setFilters((current) => ({ ...current, category: value }))} allLabel="All messages" className="w-full sm:w-auto" /><SearchBar placeholder="Search messages..." value={filters.search} onChange={(value) => setFilters((current) => ({ ...current, search: value }))} className="w-full sm:w-60" /><FilterDropdown options={[{ value: 'active', label: 'Active' }, { value: 'archived', label: 'Archived' }]} value={filters.view} onChange={(value) => setFilters((current) => ({ ...current, view: value }))} className="w-full sm:w-auto" /></div>
